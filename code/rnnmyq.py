...
def gen_poem(begin_word):
    ...
    with tf.Session() as sess:
        ...
        print(1)
        print(to_word(prediction_,vocabularies))
        for j in range(10):
            while word!=' 'and word!='。' and word!='G':        
                poem+=word           
                x=np.zeros((1,1))
            ...
            if  word=='。':
                ...
                x=np.zeros((1,1))         
                if j%9==0:
                    x[0,0]=word_int_map['天']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)
                if j%9==1:
                    x[0,0]=word_int_map['明']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)                    
                if j%9==2:
                    x[0,0]=word_int_map['满']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)                    
                if j%9==3:
                    x[0,0]=word_int_map['楼']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)                    
                if j%9==4:
                    x[0,0]=word_int_map['春']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)                    
                if j%9==5:
                    x[0,0]=word_int_map['花']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)                    
                if j%9==6:
                    x[0,0]=word_int_map['岁']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)
                if j%9==7:
                    x[0,0]=word_int_map['新']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word)
                if j%9==8:
                    x[0,0]=word_int_map['静']
                    [prediction_,state]=sess.run([prediction,last_state],feed_dict={input_data: x,initial_state:state})
                    word = to_word(prediction_,vocabularies)               
                    print(3)
                    print(word) 
